        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                if pos[0] > var.DISPLAYWIDTH * 0.3125 and pos[0] < var.DISPLAYWIDTH * 0.6425 and pos[1] > var.DISPLAYHEIGHT * 0.75 and pos[1] < var.DISPLAYHEIGHT * 0.84:
                    ...
                    player1.initXPos = round(int(var.DISPLAYWIDTH * 0.0625), -1)
                    player1.initYPos = round(int(var.DISPLAYHEIGHT * (1/12)), -1)
                    player1.xPos = round(int(player1.initXPos), -1)
                    player1.yPos = round(int(player1.initYPos), -1)
                    player2.initXPos = round(int(var.DISPLAYWIDTH * 0.9125), -1)
                    player2.initYPos = round(int(var.DISPLAYHEIGHT * (53/60)), -1)
                    player2.xPos = round(int(player2.initXPos), -1)
                    player2.yPos = round(int(player2.initYPos), -1)
                    var.introScreen = False